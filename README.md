...
```bash
git clone https://github.com/PestoTech/web3-security-challenges.git && cd web3-security-challenges && yarn install
```
...
Everyone wants to be at the top of Mount Pesto, but there's space for only one. As they say, everything can be bought with money, so can be the spot at the summit. Whoever sends the Mount Pesto Smart Contract an amount of ether that is larger than the current bribe replaces the previous climber. On such an event, the replaced climber gets paid the new bribe, making a bit of ether in the process!
...
**Check out the [Pesto Hill](contracts/PestoHill/PestoHill.sol) smart-contract and find a way to hack it.**

**Pass this [Test](test/pesto-hill.js) to win the challenge.**
...
The pool has 1 million Pesto Tokens (PST) in balance. Complete the challenge by making Bob steal all the PSTs from the pool and send them into the his account.
